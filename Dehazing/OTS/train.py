...
def _train(model, args, device):
    ...
    for epoch_idx in range(epoch, args.num_epoch + 1):
        ...
        for iter_idx, batch_data in enumerate(dataloader):
            ...
            loss = loss_content + args.fft_weight * loss_fft
            loss.backward()
            ...
            if iter_idx%eval_now==0 and iter_idx>0 and (epoch_idx>args.num_epoch-20 or epoch_idx == 1):
                save_name = os.path.join(args.model_save_dir, 'model_%d_%d.pkl' % (epoch_idx, iter_idx))
                torch.save({'model': model.state_dict()}, save_name)

                val_gopro_psnr, val_gopro_ssim = _valid(model, args, epoch_idx, device)
                print('%03d epoch \n Average GOPRO PSNR %.2f dB, Average GOPRO SSIM %.4f dB' % (epoch_idx, val_gopro_psnr, val_gopro_ssim))
                writer.add_scalar('PSNR_GOPRO', val_gopro_psnr, epoch_idx)
                writer.add_scalar('SSIM_GOPRO', val_gopro_ssim, epoch_idx)
                if val_gopro_psnr > best_psnr or (val_gopro_psnr == best_psnr and val_gopro_ssim >= best_ssim):
