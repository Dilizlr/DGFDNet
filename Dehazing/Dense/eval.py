...
def _eval(model, args, device):
    log_file_path = os.path.join(args.result_dir, "dense_psnr.txt")
    ...
    with torch.no_grad():
        ...
        for iter_idx, data in enumerate(dataloader):
            ...
            print('%d iter Name: %s PSNR_dehazing: %.2f ssim: %f' % (iter_idx + 1, name[0], psnr_val, ssim_val))
            ssim_adder(ssim_val)
